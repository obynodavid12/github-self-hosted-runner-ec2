...
resource "aws_launch_template" "ec2_launch_template" {
  ...
  user_data = base64encode(templatefile("$file/bootstrap.tmpl", { github_repo_url = var.github_repo_url, github_repo_pat_token = var.github_repo_pat_token, runner_name = var.runner_name, labels = join(",", var.labels) }))
